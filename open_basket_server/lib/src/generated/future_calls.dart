...
class _CloseBasketFutureCallDispatcher {
  _CloseBasketFutureCallDispatcher(this._invokeFutureCall);

  final _InvokeFutureCall _invokeFutureCall;

  Future<void> close(int basketId) {
    var object = _itt3vjix.CloseBasketFutureCallCloseModel(basketId: basketId);
    return _invokeFutureCall(
      'CloseBasketCloseFutureCall',
      object,
    );
  }
}
...
class CloseBasketCloseFutureCall
    ...
    implements
        _is.InvokableFutureCall<_itt3vjix.CloseBasketFutureCallCloseModel> {
  @override
  _ida.Future<void> invoke(
    _is.Session session,
    _itt3vjix.CloseBasketFutureCallCloseModel? object,
  ) async {
    if (object != null) {

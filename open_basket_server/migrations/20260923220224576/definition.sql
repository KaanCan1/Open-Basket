...
--
-- Class AnalyticsEvent as table analytics_event
--
...
--
-- Class BasketItem as table basket_item
--
...
--
-- Class Household as table household
--
...
--
-- Class HouseholdMember as table household_member
--
...
--
-- Class JoinAttempt as table join_attempt
--
...
-- Indexes
CREATE INDEX "join_attempt_user_idx" ON "join_attempt" USING btree ("userId", "attemptedAt");

--
-- Class RetiredHouseholdCode as table retired_household_code
--
CREATE TABLE "retired_household_code" (
    "id" bigserial PRIMARY KEY,
    "householdId" bigint NOT NULL,
    "code" text NOT NULL,
    "retiredAt" timestamp without time zone NOT NULL
);

-- Indexes
CREATE UNIQUE INDEX "retired_household_code_idx" ON "retired_household_code" USING btree ("code");

--
-- Class SettlementLine as table settlement_line
--
...
--
-- Class SignInCode as table sign_in_code
--
...
--
-- Foreign relations for "basket_item" table
--
...
--
-- Foreign relations for "household_member" table
--
...
--
-- Foreign relations for "retired_household_code" table
--
ALTER TABLE ONLY "retired_household_code"
    ADD CONSTRAINT "retired_household_code_fk_0"
    FOREIGN KEY("householdId")
    REFERENCES "household"("id")
    ON DELETE CASCADE
    ON UPDATE NO ACTION;
...
--
-- Foreign relations for "settlement_line" table
--
...
INSERT INTO "serverpod_migrations" ("module", "version", "timestamp")
    VALUES ('open_basket', '20260923220224576', now())
    ON CONFLICT ("module")
    DO UPDATE SET "version" = '20260923220224576', "timestamp" = now();
